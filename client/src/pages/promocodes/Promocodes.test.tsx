import axios from "axios";

import { fireEvent, screen } from "@testing-library/react";

import { IPromocode } from "../../models/IResponse";
import PromocodeStore from "../../store/mobxStore/promocodeStore/PromocodeStore";
import { renderWithRouterAndRedux } from "../../tests/renderWithRouterandRedux";

import Promocodes from "./Promocodes";

jest.mock("axios", () => {
  return {
    get: jest.mock,
    create: () => {
      return {
        interceptors: {
          request: { eject: jest.fn(), use: jest.fn() },
          response: { eject: jest.fn(), use: jest.fn() },
        },
      };
    },
  };
});

describe("Promocodes Page", () => {
  let response: IPromocode[];
  let fakeStore: any;
  beforeEach(() => {
    response = [
      {
        id: "3254501a-c052-46dc-9328-954a856ddcbc",
        name: "Тест",
        promocode: "SOTRUDNIK",
        percent: 20,
        catalog_product: {
          id: "644a5d931b666a27fd169ed6",
          name: "Эстетический уход",
          position: 1,
          __v: 0,
        },
        sub_catalog_product: {
          id: "6a98af2a-e190-4af2-a8c9-7514f5cd4c72",
          name: "Скрабы",
          position: 1,
          catalog_product: "644a5d931b666a27fd169ed6",
          __v: 0,
        },
        brand: {
          id: "7aaaeaee-6e50-4225-9ab2-15da6d802ba7",
          name: "Laboratory Cytolife",
          icon: "b350ff6b-5576-4c51-90a7-7747ec4b8c48.jpg",
          margin: 40,
        },
        products: [
          {
            id: "0e7236ed-7544-40d3-bf28-4bddfa99d8af",
            name: "Биоревитализант Catalystin, 4 мл",
            isRetailAllowed: false,
            brand: {
              id: "b9a8c4ab-3570-47a4-a75f-940f784211bd",
              name: "Vec",
            },
            images: [],
          },
          {
            id: "500290a7-09b9-4108-87c2-2bf4c0c45f88",
            name: "Геропротекторный комплекс Alinex, 2 мл",
            isRetailAllowed: false,
            brand: {
              id: "b9a8c4ab-3570-47a4-a75f-940f784211bd",
              name: "Vec",
            },
            images: [],
          },
          {
            id: "6cb32109-e2ca-4a2b-a372-a2fdd788f04e",
            name: "Мультипептидный комплекс, 2 мл",
            isRetailAllowed: false,
            brand: {
              id: "b9a8c4ab-3570-47a4-a75f-940f784211bd",
              name: "Vec",
            },
            images: [],
          },
          {
            id: "6f681ad4-d426-4607-9e30-2bda224ba67f",
            name: "Пептидный комплекс для иммунитета кожи DeAcne, 2 мл",
            isRetailAllowed: false,
            brand: {
              id: "b9a8c4ab-3570-47a4-a75f-940f784211bd",
              name: "Vec",
            },
            images: [],
          },
          {
            id: "5d6bc26f-c1a1-468e-8b50-79b61982bd6e",
            name: "Трихологический пептидный комплекс Tricholax, 2 мл",
            isRetailAllowed: false,
            brand: {
              id: "b9a8c4ab-3570-47a4-a75f-940f784211bd",
              name: "Vec",
            },
            images: [],
          },
        ],
      },
      {
        id: "f4ef5afc-c69a-4746-b94a-9d66bf627c51",
        name: "Скидка 20% на Kosmoteros Professionnel",
        promocode: "KOSMOTEROS",
        percent: 20,
        catalog_product: {
          id: "644a5d931b666a27fd169ed6",
          name: "Эстетический уход",
          position: 1,
          __v: 0,
        },
        sub_catalog_product: {
          id: "0e325ef5-b8fd-483d-b136-78895788e5a0",
          name: "Сыворотки-концентраты",
          position: 4,
          catalog_product: "644a5d931b666a27fd169ed6",
          __v: 0,
        },
        brand: {
          id: "9c44f061-6a7a-40a9-9c5c-b5c512bb5048",
          name: "V45",
          icon: "c0f061fe-8e64-4620-835a-517ce44f1c5d.jpg",
          margin: 60,
        },
        products: [
          {
            id: "0e7236ed-7544-40d3-bf28-4bddfa99d8af",
            name: "Биоревитализант Catalystin, 4 мл",
            isRetailAllowed: false,
            brand: {
              id: "b9a8c4ab-3570-47a4-a75f-940f784211bd",
              name: "Vec",
            },
            images: [],
          },
          {
            id: "500290a7-09b9-4108-87c2-2bf4c0c45f88",
            name: "Геропротекторный комплекс Alinex, 2 мл",
            isRetailAllowed: false,
            brand: {
              id: "b9a8c4ab-3570-47a4-a75f-940f784211bd",
              name: "Vec",
            },
            images: [],
          },
          {
            id: "6cb32109-e2ca-4a2b-a372-a2fdd788f04e",
            name: "Мультипептидный комплекс, 2 мл",
            isRetailAllowed: false,
            brand: {
              id: "b9a8c4ab-3570-47a4-a75f-940f784211bd",
              name: "Vec",
            },
            images: [],
          },
          {
            id: "6f681ad4-d426-4607-9e30-2bda224ba67f",
            name: "Пептидный комплекс для иммунитета кожи DeAcne, 2 мл",
            isRetailAllowed: false,
            brand: {
              id: "b9a8c4ab-3570-47a4-a75f-940f784211bd",
              name: "Vec",
            },
            images: [],
          },
          {
            id: "5d6bc26f-c1a1-468e-8b50-79b61982bd6e",
            name: "Трихологический пептидный комплекс Tricholax, 2 мл",
            isRetailAllowed: false,
            brand: {
              id: "b9a8c4ab-3570-47a4-a75f-940f784211bd",
              name: "Vec",
            },
            images: [],
          },
        ],
      },
    ];
    fakeStore = {
      ...PromocodeStore,
      promocodesM: response,
      fetchPromocodesM: () => null,
      setActiveElementM: () => null,
    };
  });

  //   it("fetchData", async () => {
  //     jest.spyOn(axios, "get").mockResolvedValue(response);
  //     renderWithRouter(<Promocodes PromocodeStore={fakeStore} />);
  //     const data = await axios.get("http://localhost:5005/promocodes");
  //     expect(data).toEqual(response);
  //     expect(axios.get).toBeCalledTimes(1);
  //   });

  //   test("click add promocode", () => {
  //     renderWithRouter(<Promocodes PromocodeStore={fakeStore} />);
  //     const addBtn = screen.getByTestId("addBtn");
  //     expect(screen.queryByTestId("addForm")).toBeNull();
  //     fireEvent.click(addBtn);
  //     expect(screen.queryByTestId("addForm")).toBeInTheDocument();
  //   });

  //   test("click edit promocode", () => {
  //     renderWithRouter(<Promocodes PromocodeStore={fakeStore} />);
  //     const tableRows = screen.getAllByTestId("tableRow");
  //     expect(screen.queryByTestId("addForm")).toBeNull();
  //     fireEvent.click(tableRows[0]);
  //     expect(screen.queryByTestId("addForm")).toBeInTheDocument();
  //   });

  //   test("click deleteIcon", () => {
  //     renderWithRouter(<Promocodes PromocodeStore={fakeStore} />);
  //     const delIcons = screen.getAllByTestId("delIcon");
  //     expect(delIcons[0]).toBeInTheDocument();
  //   });
});
